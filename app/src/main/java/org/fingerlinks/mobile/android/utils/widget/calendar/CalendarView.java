...
public class CalendarView extends LinearLayout {
    ...
    // Class
    private CalendarListener robotoCalendarListener;
    ...
    private void initializeDaysOfMonthLayout() {
        ...
        for (int i = 1; i < 43; i++) {
            ...
            dayOfMonthText.setTextSize(TypedValue.COMPLEX_UNIT_SP, TEXT_SIZE_SP);
    ...
    private void clearDayOfTheMonthStyle(Calendar currentDate) {

        if (currentDate != null) {
            ViewGroup dayOfMonthBackground = getDayOfMonthBackground(currentDate);
            dayOfMonthBackground.setBackgroundResource(android.R.color.transparent);
        }
    }
    ...
    public void unMarkDay(Date currentDate) {
        Calendar currentCalendar = getCurrentCalendar();
        currentCalendar.setTime(currentDate);
    ...
    public void markDayWithStyle(int style, Date currentDate) {
        ...
        currentCalendar.setTime(currentDate);
        ViewGroup dayOfMonthBackground = getDayOfMonthBackground(currentCalendar);
        dayOfMonthBackground.setVisibility(VISIBLE);
        int c = ContextCompat.getColor(dayOfMonthBackground.getContext(), style);
        dayOfMonthBackground.setBackgroundResource(R.drawable.circle);
        dayOfMonthBackground.getBackground().mutate().setColorFilter(c, PorterDuff.Mode.SRC);
        TextView dayOfMonthTextView = getDayOfMonthText(currentCalendar);
    ...
    public void setEventsToCalendar(List<SupportCalendarItem> calendarItems) {
        ...
        //todo clear items in calendar...
        int m = currentCalendar.get(Calendar.MONTH);
        int a = currentCalendar.get(Calendar.YEAR);
        Calendar calendarD = Calendar.getInstance();
        for (int i = 1; i <= getCurrentCalendar().getActualMaximum(Calendar.DAY_OF_MONTH); i++) {
            calendarD.set(a, m, i);
            //this.unMarkFirstUnderlineWithStyle(calendarD.getTime());
            this.unMarkDay(calendarD.getTime());
        }
        for (SupportCalendarItem item : calendarItems) {
            if (currentCalendar.getTime().getMonth() == item.getDate().getMonth() &&
                    currentCalendar.getTime().getYear() == item.getDate().getYear()) {
                //this.markFirstUnderlineWithStyle(item.getStyle(), item.getDate());
                this.markDayWithStyle(item.getStyle(), item.getDate());
            }
        }
    }

    public void unMarkFirstUnderlineWithStyle(Date currentDate) {
        Locale locale = context.getResources().getConfiguration().locale;
        Calendar currentCalendar = Calendar.getInstance(locale);
        currentCalendar.setTime(currentDate);
    ...
    public interface CalendarListener {
        ...
        void onDateSelected(Date date, SupportCalendarItem item);

    }

    public void setCalendarListener(CalendarListener robotoCalendarListener) {
    ...
    private OnClickListener onDayOfMonthClickListener = new OnClickListener() {
        ...
        public void onClick(View view) {
            ...
            if (robotoCalendarListener == null) {
                ...
                robotoCalendarListener.onDateSelected(calendar.getTime(), calendarItem(calendar.getTime()));
            }
        }
    };

    private SupportCalendarItem calendarItem(Date date) {
        for (SupportCalendarItem calendarItem : calendarItems) {
            if (DateUtils.isSameDay(calendarItem.getDate(), date)) {
                return calendarItem;
            }
        }
        return null;
